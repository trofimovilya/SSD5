...
* @Last Modified time: 2014-04-08 11:36:17
*/
...
int grid::recoursiveCount(int row, int col) {
    if (row >= 0 && row < rows && col >= 0 && col < cols &&
        ...
        recoursiveCount(row, col - 1);
        recoursiveCount(row, col + 1);
        recoursiveCount(row - 1, col);
        recoursiveCount(row - 1, col - 1);
        recoursiveCount(row - 1, col + 1);
        recoursiveCount(row + 1, col);
        recoursiveCount(row + 1, col - 1);
        recoursiveCount(row + 1, col + 1);
...
int grid::count (int row, int col) {
    ...
    return recoursiveCount(row, col);
}